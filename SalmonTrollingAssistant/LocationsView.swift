    ...
    var body: some View {
        NavigationView {
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button(action: {
                        // Always set the coordinate regardless of which tab we're on
        ...
        .sheet(isPresented: $showingForecast) {
            if let location = selectedLocation {
                // Use the EnhancedForecastView from ContentView.swift
                ForecastViewWrapper(location: location)
            }
        }
        ...
        NavigationView {
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    // See Recommended Gear Button - Made more prominent
                    Button(action: {
                        // Dismiss this sheet
